...
function formatSlack(event: NotificationEvent, p: NotificationPayload): object {
  ...
  if (p.output || p.summary) lines.push(`Output: ${(p.output ?? p.summary ?? "").slice(0, 200)}`);
...
function formatTeams(event: NotificationEvent, p: NotificationPayload): object {
  ...
  if (p.output || p.summary) facts.push({ name: "Output", value: (p.output ?? p.summary ?? "").slice(0, 200) });
...
function formatGeneric(event: NotificationEvent, p: NotificationPayload): object {
  return {
    event,
    status: p.status ?? event.split(".").at(-1),
...
function buildHeaders(event: NotificationEvent | string, body: string, secret?: string): Record<string, string> {
  const safeEvent = String(event).replace(/[^\x20-\x7E]/g, "").slice(0, 128) || "custom";
  const headers: Record<string, string> = {
    "Content-Type": "application/json",
    "X-NanoOrch-Event": safeEvent,
  };
  if (secret) {
...
export async function dispatchToChannel(channelId: string, label: string, text: string): Promise<void> {
  try {
    ...
    const cfg = ch.config as { url?: string; secret?: string } | null;
    if (!cfg?.url) return;
    ...
    if (ch.type === "slack") {
      ...
      body = JSON.stringify({ event: "direct.message", label, text, timestamp: new Date().toISOString() });
    }
    ...
    try {
      const res = await fetch(cfg.url, {
        method: "POST",
        headers: buildHeaders("direct.message", body, cfg.secret),
        body,
